...
public class ErrorResponseInterceptor {
    ...
    private ErrorResponse problemDelegator(Exception exception, Locale locale) {
        if (exception instanceof CustomApiException)
            ...
            return problemDelegator((AuthenticationException) exception);
        else
    ...
    private ErrorResponse problemDelegator(AuthenticationException authenticationException) {
        ErrorResponse response = new ErrorResponse();
        response.setStatusCode((short) HttpServletResponse.SC_UNAUTHORIZED);
